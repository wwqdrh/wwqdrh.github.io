...
docker service create --name code-server \
    ...
    --replicas 1 \
    --network dev \
    --mount type=volume,src="codeserver_config",dst="$userhome/.config" \
    --mount type=volume,src="codeserver_data",dst="$userhome/.local" \
    --mount type=bind,src="$userhome/project",dst="$userhome/project" \
    "$image"
