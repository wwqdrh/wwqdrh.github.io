...
read network && docker create network $network --driver overlay
if [ $? -ne 0 ]; then
    echo "网络创建失败"
    exit 1
fi
...
read password && echo $password | docker secret create CODE_SERVER_PWD -
if [ $? -ne 0 ]; then
    echo "创建密码失败"
    exit 1
fi
...
read token && echo $token | docker secret create GITHUB_TOKEN -
if [ $? -ne 0 ]; then
...
echo "添加hosts 127.0.0.1 code-server >> /etc/hosts"
