...
def get_tang_url() -> str:
    ids = [0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000] + \
        [i * 1000 for i in range(10, 58)]
    
    return f"https://raw.githubusercontent.com/wwqdrh/chinese-poetry/master/json/poet.tang.{random.sample(ids, 1)[0]}.json"


class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        try:
            response = requests.get(get_tang_url())
            try:
                data = response.json()
